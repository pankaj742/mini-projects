...
def mainloop():
    ...
    while(True):
        ...
        button("Randomize To solve",350,550,150,60,sunehra,thoda_sunehra,"randomize2")
        drawSide()
        #first_indicator()
...
def randomize2():
    ...
    nsteps=rno
    while(i<=rno):
        actions=list(side[nonpos//8][nonpos%8]["actions"])
        random.shuffle(actions)
        action=actions[0]
        if action=="left":
            (side[(nonpos-8)//8][(nonpos-8)%8]["tag"],side[(nonpos)//8][(nonpos)%8]["tag"])=(side[(nonpos)//8][(nonpos)%8]["tag"],side[(nonpos-8)//8][(nonpos-8)%8]["tag"])
            nonpos=nonpos-8
        elif action=="right":
            (side[(nonpos+8)//8][(nonpos+8)%8]["tag"],side[(nonpos)//8][(nonpos)%8]["tag"])=(side[(nonpos)//8][(nonpos)%8]["tag"],side[(nonpos+8)//8][(nonpos+8)%8]["tag"])
            nonpos=nonpos+8
        elif action=="up":
            (side[(nonpos-1)//8][(nonpos-1)%8]["tag"],side[(nonpos)//8][(nonpos)%8]["tag"])=(side[(nonpos)//8][(nonpos)%8]["tag"],side[(nonpos-1)//8][(nonpos-1)%8]["tag"])
            nonpos=nonpos-1
        elif action=="down":
            (side[(nonpos+1)//8][(nonpos+1)%8]["tag"],side[(nonpos)//8][(nonpos)%8]["tag"])=(side[(nonpos)//8][(nonpos)%8]["tag"],side[(nonpos+1)//8][(nonpos+1)%8]["tag"])
            nonpos=nonpos+1
...
def getcost(state):
    ...
    for i in range(64):
        if((i != nonpos) and (pos[i]-1 != i)):
            h1 += 1
        if(i != nonpos):
            h2 += (max((pos[i]-1)//8,i//8)-min((pos[i]-1)//8,i//8))+(max((pos[i]-1)%8,i%8)-min((pos[i]-1)%8,i%8))
    return(h1+h2)
...
def checkvisited(state):
    ...
    for i in range(len(explored)):
        tstateno=explored[i]["stateno"]
        tstate=states[tstateno]
        if(state["pos"]==tstate["pos"]):
            return True
    for j in range(len(frontier)):
        tstateno=frontier[j]["stateno"]
        tstate=states[tstateno]
        if(state["pos"]==tstate["pos"]):
            return True
##        elif(state["ncost"]==frontier[j]["cost"]):
##            count += 1
##            if(count ==2):
##                return True
    return False
...
def applyactions(state):
    ...
    for action in actions:
        ...
        if action == "up":
            ...
            newstate["ncost"]=getcost(newstate)
...
##                print("added to frontier in up :"+str(newstate["nonpos"]))
##                print(frontiernode)
            ...
            if(checkvisited(newstate) == False):
                ...
                frontiernode["cost"]=newstate["cost"]+getcost(newstate)
                addfrontier(frontiernode)
##                print("added to frontier in down :"+str(newstate["nonpos"]))
##                print(frontiernode)
                states[stateno]=newstate
            ...
            if(checkvisited(newstate) == False):
                ...
                frontiernode["cost"]=newstate["cost"]+getcost(newstate)
                addfrontier(frontiernode)
##                print("added to frontier in left :"+str(newstate["nonpos"]))
##                print(frontiernode)
                states[stateno]=newstate
            ...
            if(checkvisited(newstate) == False):
                ...
                frontiernode["cost"]=newstate["cost"]+getcost(newstate)
                addfrontier(frontiernode)
##                print("added to frontier in right :"+str(newstate["nonpos"]))
##                print(frontiernode)
                states[stateno]=newstate
...
def start():
    ...
    #print(states)
    #start searching
    depthlimit=getcost(state)
    i=1
    flag=0
    print("depth limit:"+str(depthlimit))
    while(i<=depthlimit*nsteps):
        if len(frontier) == 0:
            print("failed nothing to explore")
            flag=1
            break
        node=getfrontier()
        print(node)
        ...
        if(getcost(states[node["stateno"]]) == 0):
            print(states[node["stateno"]]["path"])
            print("success")
            path=states[node["stateno"]]["path"][:]
            solveboard(path)
            states={}
            frontier=[]
            explored=[]
    ...
    for i in range(8):
        ...
        for j in range(8):
            ...
            if(box["pos"] in (1,2,3,4,5,6)):
                box["actions"]=("right","up","down")
            elif(box["pos"] in (15,23,31,39,47,55)):
                box["actions"]=("right","up","left")
            elif(box["pos"] in (57,58,59,60,61,62)):
                box["actions"]=("down","up","left")
            elif(box["pos"] in (8,16,24,32,40,48)):
                box["actions"]=("right","down","left")
            elif(box["pos"] == 0):
                box["actions"]=("right","down")
            elif(box["pos"] == 7):
                box["actions"]=("right","up")
            elif(box["pos"] == 63):
                box["actions"]=("left","up")
            elif(box["pos"] == 56):
                box["actions"]=("down","left")
            else:
                box["actions"]=("right","down","up","left")
            side[i].append(box)
